 ...
 * 
 * Usage: npx tsx scripts/typescript/payment/payment-clean.ts [--full]
 */
...
async function cleanupPaymentData() {
  ...
  try {
    ...
    const transactionsDropped = collectionNames.includes('transactions');
    ...
    // ✅ CRITICAL: Recreate unique index on transactions.metadata.externalRef after dropping collection
    // This ensures duplicate protection is always in place, even if service is already running
    if (transactionsDropped) {
      console.log('\n🔧 Recreating unique index on transactions.metadata.externalRef...');
      try {
        const transactionsCollection = db.collection('transactions');
        
        // Create unique index (collection was just dropped, so no conflicts)
        ...
        console.log('   ✅ Created unique index on metadata.externalRef');
      } catch (indexError: any) {
        // If collection doesn't exist yet, that's OK - service will create it
        if (indexError.message?.includes('does not exist') || indexError.codeName === 'NamespaceNotFound') {
          console.log('   ⏭️  Transactions collection does not exist yet - index will be created when collection is created');
        } else {
          console.log(`   ⚠️  Could not create unique index: ${indexError.message}`);
          console.log('      The service startup code will create it on next operation');
        }
      }
    }
    ...
    // Clean up sessions from auth_service database
    console.log('\n🧹 Cleaning sessions from auth_service database...');
    const authClient = new MongoClient('mongodb://localhost:27017/auth_service?directConnection=true');
    try {
      await authClient.connect();
      const authDb = authClient.db();
      
      // Delete all sessions
      const sessionsCount = await authDb.collection('sessions').countDocuments();
      if (sessionsCount > 0) {
      ...
      const refreshTokensCount = await authDb.collection('refresh_tokens').countDocuments();
      if (refreshTokensCount > 0) {
        await authDb.collection('refresh_tokens').deleteMany({});
        console.log(`   ✅ Deleted ${refreshTokensCount} refresh tokens`);
      } else {
        console.log('   ⏭️  No refresh tokens to delete');
      }
    } catch (authError: any) {
      console.log(`   ⚠️  Could not clean auth_service sessions: ${authError.message}`);
      console.log('      This is OK if auth_service is not running or database is not accessible');
    } finally {
    ...
    console.log('✅ Sessions and refresh tokens cleaned!');
    if (fullCleanup) {
      ...
      console.log('   All sessions and refresh tokens have been deleted - users will need to log in again.');
    } else {
