    ...
    try {
      ...
      if (healthRes.data) {
        ...
        if (secret) {
          ...
          if (testRes.errors) {
            console.log(`  ⚠️  ${config.name}: No secret configured (set ${key.toUpperCase()}_JWT_SECRET)`);
            continue;
          }
          console.log(`  ✅ ${config.name} at ${config.url} (no auth required)`);
        }
        
        discovered.push({ config, token });
      }
    } catch (err) {
      console.log(`  ❌ ${config.name}: Not available (${(err as Error).message})`);
    }
  }
...
function printResult(result: BenchmarkResult) {
  console.log(`
┌─────────────────────────────────────────────────────────────┐
│ ${result.name.padEnd(57)} │
├─────────────────────────────────────────────────────────────┤
│ Iterations: ${String(result.iterations).padEnd(45)} │
│ Total Time: ${String(result.totalMs + 'ms').padEnd(45)} │
│ Avg:        ${String(result.avgMs + 'ms').padEnd(45)} │
│ Min:        ${String(result.minMs + 'ms').padEnd(45)} │
│ Max:        ${String(result.maxMs + 'ms').padEnd(45)} │
│ P95:        ${String(result.p95Ms + 'ms').padEnd(45)} │
│ RPS:        ${String(result.rps + ' req/sec').padEnd(45)} │
└─────────────────────────────────────────────────────────────┘`);
}
...
║                           BENCHMARK SUMMARY                                  ║
╠═══════════════════════════════════╦═══════════╦═══════════╦═════════════════╣
║ Service / Test                    ║ Avg (ms)  ║ P95 (ms)  ║ RPS             ║
╠═══════════════════════════════════╬═══════════╬═══════════╬═════════════════╣`);
  
  for (const { service, results: svcResults } of results) {
    console.log(`║ ${service.padEnd(33)} ║           ║           ║                 ║`);
    for (const r of svcResults) {
      const name = `  ${r.name.slice(0, 31)}`;
      console.log(`║ ${name.padEnd(33)} ║ ${String(r.avgMs).padStart(9)} ║ ${String(r.p95Ms).padStart(9)} ║ ${String(r.rps).padStart(15)} ║`);
    }
  }
...
async function benchmarkService(
  ...
  console.log(`\n⏳ Running concurrent test (${workers} workers, ${duration / 1000}s)...`);
...
║             GENERIC MICROSERVICE BENCHMARK                                 ║
╠═══════════════════════════════════════════════════════════════════════════╣
║  Tests all available microservices with:                                   ║
║  • Health check (baseline latency)                                         ║
║  • List queries (read performance)                                         ║
║  • Create mutations (write performance with sagas)                         ║
