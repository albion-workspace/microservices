 ...
 * 
 * Usage: npx tsx scripts/payment-test-duplicate.ts
 */
...
async function getFromUserId(): Promise<string> {
  // Get payment-provider user ID by email from auth service
  const query = `
  ...
  const result = await graphql(query, { first: 100 }, AUTH_SERVICE_URL);
  const providerUser = result.users?.nodes?.find((u: any) => u.email === CONFIG.testFromUserEmail);
...
async function approveDeposit(transactionId: string): Promise<any> {
  ...
  return graphql(query, { transactionId });
}
