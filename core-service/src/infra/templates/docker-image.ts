...
// 
// NOTE: Version configuration is in gateway/configs/infra.json
// This template accepts nodeVersion as a required parameter
// ═══════════════════════════════════════════════════════════════════
 ...
 * 
 * @param nodeVersion - Node.js version (from gateway/configs/infra.json)
 */
export function generateCoreBaseDockerfile(nodeVersion: string): string {
  return `# ═══════════════════════════════════════════════════════════════════
...
export function generateServiceDockerfile(config: DockerConfig): string {
  const {
    ...
    port = 3000,
    healthPath = '/health',
    nodeVersion,  // Required - should come from gateway/configs/infra.json
    entryPoint = 'dist/index.js',
    localDependencies = [],
  } = config;
  
  if (!nodeVersion) {
    throw new Error('nodeVersion is required - should come from gateway/configs/infra.json');
  }
