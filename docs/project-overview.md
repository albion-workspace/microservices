...
| **Config loadConfig pattern** | **Done** | All five services’ `config.ts`: same pattern `getConfigWithDefault(SERVICE_NAME, 'port', { brand, tenantId }) ?? (gateway fallback) ?? default` for port, serviceName, nodeEnv, corsOrigins, jwt, database. Keys and defaults are service-specific. | Optional helper in core-service, e.g. `getServiceConfigKey(service, key, defaultVal, options?)` that encapsulates service + optional gateway fallback. Would shorten each loadConfig and ensure consistent fallback semantics. Only do if adding a new service or touching all configs. |
